package com.zjp.commonutils.utils.download;
...
public class DownloadTask implements Runnable {
    ...
    DownloadTask(String url, long lowerBound, long upperBound, DownloadFile downloadFile,
                 AtomicBoolean canceled, int threadID, Object lock) {
        this.url = url;
    ...
    public void run() {
        ...
        try {
            ByteBuffer buffer = ByteBuffer.allocate(1024 * 8); // 2MB
            input = connect();
            ...
            if (canceled.get()) {
                System.out.println("[线程" + threadId + "]下载失败: 任务取消");
            }
        } catch (IOException e) {
            canceled.set(true);
            System.err.println("ERROR: [线程" + threadId + "]遇到错误[" + e.getMessage() + "]，结束下载");
        } finally {
            if (input != null) {
                try {
                    input.close();
